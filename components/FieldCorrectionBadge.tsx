...
export default function FieldCorrectionBadge({ fieldKey, corrections, onCorrect }: FieldCorrectionBadgeProps) {
  ...
  const statusColor = latest.status === "APPROVED" ? "var(--positive)" : latest.status === "REJECTED" ? "var(--negative)" : "var(--warning, #F5A623)";
  ...
  return (
    <span className="correction-badge" title={tooltip} style={{ position: "relative", display: "inline-flex", alignItems: "center", gap: 2 }}>
      ...
      <span style={{
        ...
        background: `${statusColor}20`, color: statusColor, fontSize: 9, fontWeight: 700, cursor: "help", flexShrink: 0,
