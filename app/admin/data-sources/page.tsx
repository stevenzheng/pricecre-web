...
interface DataSource {
  ...
  fieldMap: any; crawlConfig: any; isActive: boolean; priority: number;
  ...
  return (
    <div className="admin-content-inner">
      <div className="admin-page-header" style={{ display: "flex", justifyContent: "space-between" }}>
        <div>
          <h1 className="admin-page-title">数据源</h1>
          <p className="admin-page-desc">{sources.length} 个数据源 · {activeCount} 活跃 — 商业地产数据采集网络</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleSave} style={{ background: "#fff", border: "1px solid #e5edf5", borderRadius: 6, padding: 16, marginBottom: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div><label style={{ fontSize: 11, fontWeight: 500, color: "#64748d" }}>名称</label><input style={{ width: "100%", padding: "7px 12px", border: "1px solid #e5edf5", borderRadius: 4, fontSize: 13 }} value={edit.name || ""} onChange={e => setEdit({...edit, name: e.target.value})} required /></div>
          <div><label style={{ fontSize: 11, fontWeight: 500, color: "#64748d" }}>URL</label><input style={{ width: "100%", padding: "7px 12px", border: "1px solid #e5edf5", borderRadius: 4, fontSize: 13, fontFamily: "monospace" }} value={edit.url || ""} onChange={e => setEdit({...edit, url: e.target.value})} required /></div>
          <div><label style={{ fontSize: 11, fontWeight: 500, color: "#64748d" }}>方式</label><select style={{ width: "100%", padding: "7px 12px", border: "1px solid #e5edf5", borderRadius: 4, fontSize: 13 }} value={edit.sourceType} onChange={e => setEdit({...edit, sourceType: e.target.value})}>{Object.entries(typeLabel).map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select></div>
          <div><label style={{ fontSize: 11, fontWeight: 500, color: "#64748d" }}>优先级</label><input style={{ width: "100%", padding: "7px 12px", border: "1px solid #e5edf5", borderRadius: 4, fontSize: 13 }} type="number" min={0} max={100} value={edit.priority ?? 0} onChange={e => setEdit({...edit, priority: parseInt(e.target.value) || 0})} /></div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        {types.map(t => (
          <button key={t} onClick={() => setFilter(t)} style={{
            padding: "4px 12px", borderRadius: 4, border: "1px solid", fontSize: 11, cursor: "pointer",
          ...
          }}>{t === "all" ? `全部 (${sources.length})` : `${typeLabel[t] || t} (${sources.filter(s => s.sourceType === t).length})`}</button>
        ))}
      </div>
          ...
          <table className="str-table">
            <thead>
              <tr>
                ...
                <th style={{ textAlign: "right", width: 140 }}></th>
              </tr>
            </thead>
            <tbody>
              {sources.map(s => (
                <tr key={s.id} style={{ opacity: s.isActive ? 1 : 0.5 }}>
                  ...
                  <td style={{ textAlign: "right" }}>
                    ...
                    <button onClick={async () => { if(confirm("删除？")) { await fetch(`/api/admin/data-sources/${s.id}`, {method:"DELETE"}); fetchSources(); }}} className="btn-danger">删除</button>
                  </td>
                </tr>
              ))}
            </tbody>
