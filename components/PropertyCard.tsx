          ...
          <div className="p-4 sm:p-5" style={{ background: "var(--panel)" }}>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                ...
                <button onClick={(e) => { e.stopPropagation(); if (typeof window !== "undefined") { const evt = new CustomEvent("open-wechat-card", { detail: { projectName: property.projectName, city: property.city, district: property.district, faceRent: property.faceRent, propertyType: typeLabel, indicators: sortedFields.filter(f => !f.isLocked).slice(0, 9).map(f => ({ label: f.label, value: formatValue(f.value, f.format) })) }}); document.dispatchEvent(evt); }}} className="ml-auto p-1.5 rounded-lg hover:bg-[var(--panel)]" aria-label="微信分享"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg></button>
                  </div>
                )}
              </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-0.5">
                      <span className="text-[11px] font-medium cursor-help hover:underline decoration-dotted" style={{ color: "var(--text-muted)" }} onMouseEnter={(e) => { const target = e.currentTarget; const expl = indicatorExplanations[field.key]; if (expl) { const tip = document.createElement("div"); tip.className = "indicator-tooltip"; tip.textContent = expl; tip.style.cssText = "position:fixed;z-index:9999;max-width:280px;padding:10px 14px;background:var(--bg-surface);color:var(--text);border:1px solid var(--line);border-radius:10px;font-size:11px;line-height:1.5;box-shadow:0 4px 20px rgba(0,0,0,0.15);pointer-events:none;"; document.body.appendChild(tip); const rect = target.getBoundingClientRect(); tip.style.left = Math.min(rect.left, window.innerWidth - 290) + "px"; tip.style.top = (rect.bottom + 6) + "px"; target._tooltip = tip; }}} onMouseLeave={(e) => { const t = (e.currentTarget as any)._tooltip; if (t) { t.remove(); (e.currentTarget as any)._tooltip = null; } }}>
                        {field.label}
                      </span>
                      <span
