...
> 2026-06-11 第八批（复核 DeepSeek 修改 + 新需求，已由 Claude 全面接管）：
> **复核结论**：DeepSeek 做对的已保留（报告蓝logo/QR挪位/态势报告等改名/verify重复邮箱提示/重置密码接口骨架）；做坏的已修复——① **`lib/prisma.ts` 回滚**：它显式传 `datasources.url` 导致 Prisma 实例化可能抛错→所有 API 500→仪表盘数据全零（这就是"Dashboard数据不显示"的根因）。② **忘记密码两处必现 bug**：验证码只打 console 从不发邮件、重复请求因唯一键必失败——已改为 upsert + 真实发送重置邮件。
> **新增/完成**：③ 注册福利落库：验证通过即创建 3 次解锁额度 + 20 次 AI 对话额度；注册验证码邮件中新增高亮福利卡片（蓝绿渐变框）。④ 邀请奖励同步写入 UserCredit（真实扣减表）。⑤ 后台全部页面全屏自适应（去掉 1280/1200/1100/1000/960 宽度上限）。⑥ 后台统一加载态：居中旋转环（.bw-loading，覆盖全部页面）。⑦ 后台侧边栏 Logo 换为前台同款蓝色 SVG。⑧ 数字/英文字体改 Stripe 风格：比例宽 + tabular-nums 等宽数字（注：Stripe 的 Söhne 为商业字体不可分发，字体栈优先 Söhne/Inter，本机未装时回落系统字体，效果接近）。⑨ 「夜间模式」「返回前台」移至后台右上角悬浮操作条（移动端自动下移避开顶栏）。⑩ 报告页分隔线与「资产价值指标」标题色改蓝。⑪ 批量管理：数据源（多选+批量启用/停用/删除）、租金核验（多选+批量通过/驳回，仅待审条目可选）、资产数据管理列表视图（多选+批量删除，演示数据不可选）。⑫ 忘记密码入口加到前台「我的」登录界面（密码框下方）。无 schema 变更。
> **验证要点**：仪表盘数字恢复显示；注册新邮箱收到带福利高亮的验证码邮件且验证后额度为 3+20；忘记密码能收到邮件并成功重置；后台任意页面右上角可切主题/返回前台；数据源页多选后批量按钮出现。

# 给 DeepSeek 的旧部署说明（已废止，部署改为直接 git push 触发）
