...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      setError(result.error === "CredentialsSignin" ? "账号或密码错误" : result.error);
      setLoading(false);
    } else {
      ...
      <div className="w-full max-w-sm p-6 card-dark rounded-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <input
              type="email"
              className="w-full mt-1 px-3 py-2 text-sm rounded-sm border outline-none transition-colors focus:border-[var(--accent)]"
              style={{ background: "var(--bg-input)", borderColor: "var(--line)", color: "var(--text)" }}
              value={email}
          ...
          <button type="submit" disabled={loading} className="btn-primary w-full rounded-sm text-sm">
            {loading ? "登录中..." : "登录"}
          </button>
        </form>
      </div>
